...
if (typeof window !== 'undefined') {
  const isIgnorableDbError = (err: any) => {
    ...
    if (typeof err === 'string') {
      ...
      msg = `${err?.message || ''} ${err?.reason || ''} ${err?.name || ''} ${err?.code || ''} ${String(err)}`;
    }
  ...
  window.addEventListener(
    'error',
    (event) => {
      if (isIgnorableDbError(event.error) || isIgnorableDbError(event.message) || isIgnorableDbError(event)) {
  ...
  window.addEventListener('unhandledrejection', (event) => {
    if (isIgnorableDbError(event.reason) || isIgnorableDbError(event)) {
      event.preventDefault();
      event.stopImmediatePropagation();
    }
  });
